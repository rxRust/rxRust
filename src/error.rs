...
pub trait WithErrByRef<T, R> {
  type Err;
  fn call_with_err(&mut self, v: &T) -> Result<R, Self::Err>;
}
...
{
  type Err = E;
  fn call_with_err(&mut self, v: &T) -> Result<R, Self::Err> { Ok(self.0(v)) }
}
...
where
  N: FnMut(&T) -> Result<R, E>,
{
  type Err = E;
  fn call_with_err(&mut self, v: &T) -> Result<R, Self::Err> { self.0(v) }
}
